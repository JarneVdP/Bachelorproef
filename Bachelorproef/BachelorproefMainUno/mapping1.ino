...
void mapp(double x, float heading, double camera_y, float camera_heading) { //naar sample rijden
  ...
  if (camera_heading != 0 && statement == 0) // check if sample is niet voor de robot
  {
    if (heading >= camera_heading ) //rechts draaien
    {
      ...
      if (heading < camera_heading + error_h) { //doel bereikt
        statement = 1;
        Serial.println("stilstand1");
      }
      Serial.println("one");
    }
    ...
    if (statement == 1) { //stilstand , zonder werkte het niet
      //delay(5000);
      if (currentMillis - previousMillis >= interval) {
        previousMillis = currentMillis;
  ...
  //x richting
  else if ( x < camera_y - error_x  ) { // hoek is in orde, rij vooruit naar sample
    if (currentMillis - previousMillis >= interval) {
      previousMillis = currentMillis;
      vooruit();
      Serial.println("BG3");
    ...
    if (currentMillis - previousMillis >= interval) { //stilstand
      previousMillis = currentMillis;
      stilstand();
    }
  }
