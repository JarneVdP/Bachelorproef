...
while True:
    ...
    if ids is not None:
        ...
        tvec_str = "id=%s x=%4.0f  y=%4.0f t=%4.0f"%(ids, cX/10, cY/10, delta_totaltime.seconds)
        ...
        if delta.seconds >= 1.5:
            ...
            sendserial(ids, cX/10, cY/10, temphoek)
            t = dt.datetime.now()
        ...
        if time.time() - prevTime > 1.5:
            sendToArduino(str(tvec_str))
            prevTime = time.time()
        """
